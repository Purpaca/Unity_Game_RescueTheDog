...
namespace AssetManagement.VersionControl
{
    public class Utils
    {
        ...
        /*
        public static void CheckAssets(VersionInfo info) 
        {
            bool ok = true;
            foreach(AssetInfo assetInfo in info.assets) 
            {
                var fullPath = Path.Combine(AssetBundleManager.AssetBundlesStoragePath, assetInfo.name);
                string md5 = GetAssetMD5(fullPath);
