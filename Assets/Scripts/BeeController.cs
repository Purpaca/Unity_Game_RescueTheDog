...
public class BeeController : DogKillingCapable
{
    ...
    /// <param name="forceOverride">是否强制覆写此蜜蜂的目标，即使它已经有目标？</param>
    public void SetTarget(DogController target, bool forceOverride = true)
    {
        if (target != null && !ReferenceEquals(_target, target) && forceOverride)
        {
            return;
        }
    ...
    private IEnumerator BeeMovementBehaviour()
    {
        ...
        while (true)
        {
            if (_target == null)
            {
                ...
                {
                    if (colliders[i].GetComponent<DogController>() != null)
                    {
                        _target = colliders[i].GetComponent<DogController>();
                        break;
                    }
                }
            }
            ...
            {
                // 1.若蜜蜂没有碰到障碍物（包括玩家绘制的线条以及狗狗）时，蜜蜂直接朝向着狗狗所在的位置移动
    ...
    protected override void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        // 2.若蜜蜂在前侧与左右侧碰撞到障碍物（包括玩家绘制的线条以及狗狗）时，蜜蜂会沿着当前碰撞点法线的切线方向随机移动
